...
static int readLines(const char *fileName, char *lines[], int max_line) {
  ...
  fclose(file);
    spdlog::info("There are {} lines", i);
  return i;
}

static int loadLabelName(const char *locationFilename, char *label[]) {
    spdlog::info("load lable {}", locationFilename);
...
int init_post_process(std::string &label_path) {
  ...
  if (ret < 0) {
      spdlog::error("Load {} failed!", label_path);
    return -1;
  }
  return 0;
}
    ...
    for (int j = 0; j < grid_w; j++) {
      ...
      for (int c = 0; c < num_labels; c++) {
        if ((score_tensor[offset] > score_thres_i8) &&
          ...
          max_score = score_tensor[offset];
          max_class_id = c;
          //            spdlog::info("threshold is {}, {} score is {}",
          //                              threshold,
          //                              coco_cls_to_name(c),
...
int post_process_pose(rknn_app_context_t *app_ctx, rknn_output *outputs,
  ...
  od_results->count = last_count;
    spdlog::info("valid count: {}, results count: {}", validCount,
                     od_results->count);
  return 0;
}
...
int post_process_v10_detection(rknn_app_context_t *app_ctx,
  ...
  if (validCount <= 0) {
    ...
    spdlog::info("valid count is {}", validCount);
  int last_count = 0;
  od_results->count = 0;
...
int post_process_obb(rknn_app_context_t *app_ctx, rknn_output *outputs,
  ...
  for (auto c : class_set) {
      spdlog::debug("processing nms, class id is {}", coco_cls_to_name(c));
  ...
  od_results->count = last_count;
    spdlog::info(" result count: {}", last_count);
  return 0;
}
